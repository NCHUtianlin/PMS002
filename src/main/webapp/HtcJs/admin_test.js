$(function(){
	...
	//工具集
	admin_test_manager_tools={
		refresh:function(){
				$('#admin_test').datagrid('reload');
			},
		search:function(){
			$('#admin_test').datagrid({
				loader:function(param,success,error){
					var options=$("#admin_test").datagrid('options');
					var pageNow=options.pageNumber;
					var pageSize=options.pageSize;
					var search=$('#admin_test_search').val();
					var request_data={
							"parameter":search,
							"pageNow":pageNow,
							"pageSize":pageSize
					...
					$.ajax({
						...
						success:function(data){
							...
							success(data);
							$('#admin_test').datagrid('loaded');
							$('#admin_test').datagrid('resize');
						},
	...
	$("#admin_test").datagrid({
		...
		rownumbers:true,
		fitColumns:true,
		toolbar:"#admin_test_manager_tool",
		loader:function(param,success,error){
			var options=$("#admin_test").datagrid('options');
			var pageNow=options.pageNumber;
			var pageSize=options.pageSize;
