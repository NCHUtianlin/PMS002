$(function(){
	...
	//工具集
	user_shengchanwancheng_manager_tools={
			// 重新刷新数据
		refresh:function(){
				$('#user_shengchanwancheng').datagrid('reload');
			},
		search:function(){
			$('#user_shengchanwancheng').datagrid({
					loader:function(param, success, error) {
						var search = $("input[name='user_shengchanwancheng_search']").val();
						var date_from=$("input[name='user_shengchanwancheng_date_from']").val();
						var date_to=$("input[name='user_shengchanwancheng_date_to']").val();
						...
						$.ajax({
									...
									success : function(data) {
										...
										success(data);
										$('#user_shengchanwancheng').datagrid('loaded');
										$('#user_shengchanwancheng').datagrid('resize');
									},
		...
		add:function(){
			$('#user_shengchanwancheng_add').dialog('open');
		},
		redo:function(){
				$('#user_shengchanwancheng').datagrid('unselectAll');
		},
		update:function(){
				var rows=$('#user_shengchanwancheng').datagrid('getSelections');
				if(rows.length>1){
					...
					$.messager.alert("警告!","请至少选择一条记录!","warning");
						...
						$('#user_shengchanwancheng_update').form('load',{
	...
	$("#user_shengchanwancheng").datagrid({
		...
		rownumbers:true,
		fitColumns:true,
		toolbar:"#user_shengchanwancheng_manager_tool",
		loader:function(param,success,error){
			var options=$("#user_shengchanwancheng").datagrid('options');
			var pageNumber=options.pageNumber;
			var pageSize=options.pageSize;
			var request = {"parameter":"","pageNow":pageNumber,"pageSize":pageSize};
	...
	//定义用户添加弹窗
	$('#user_shengchanwancheng_add').dialog({
		width:480,
		title:'添加用户',
		height:420,
		modal:true,
		...
		buttons:[{
			...
			iconCls:'icon-ok',
			handler:function(){
				if($('#user_shengchanwancheng_add').form('validate')){
					...
					$.ajax(
							{
								...
								$('#user_shengchanwancheng_add').dialog('close').form('reset');
								$('#user_shengchanwancheng').datagrid('reload');
								$('#user_shengchanwancheng').datagrid('resize');
								},
								error: function(XMLHttpRequest,status,errorThrown){//请求完成最终执行的函数
	...
	//定义用户修改弹窗
	$('#user_shengchanwancheng_update').dialog({
		width:480,
		title:'修改用户',
		height:420,
		modal:true,
		...
		buttons:[{
			...
			iconCls:'icon-ok',
			handler:function(){
				if($('#user_shengchanwancheng_update').form('validate')){
					...
					$.ajax({
						...
						success:function(data){	
							$.messager.progress('close');
								...
								$('#user_shengchanwancheng_update').dialog('close').form('reset');
								$('#user_shengchanwancheng').datagrid('reload');
								$('#user_shengchanwancheng').datagrid('resize');
						},
						error: function(XMLHttpRequest,status,errorThrown){//请求完成最终执行的函数
