$(function(){
	...
	user_productType_manager_tools={
		...
		search:function(){
			$('#user_productType').datagrid({
					loader:function(param, success, error) {
						var options=$("#user_productType").datagrid('options');
						var pageNumber=options.pageNumber;
						var pageSize=options.pageSize;
						var search = $('#user_productType_search').val();
						var request_data = {
							"parameter" : search,
						...
						$.ajax({
							...
							success : function(data) {
								...
								$('#user_productType').datagrid('resize');
							},
		...
		update:function(){
				...
				$('#user_productType_update').form('load',{
					user_productType_id_update:rows[0]._id,
					user_productType_MAC_update:rows[0].MAC_update,
					user_productType_productTypeName_update:rows[0].productTypeName,
					user_productType_hardwareVersion_update:rows[0].hardwareVersion,
					user_productType_softwareVersion_update:rows[0].softwareVersion,
					user_productType_macNumber_update:rows[0].macNumber,
					user_productType_productTypeState_update:rows[0].productTypeState,
				}).dialog('open');				
					}
		},
	};
	...
	//产品类型名称输入框
	$("#user_productType_productTypeName_add").textbox({
		required:true,
		missingMessage:"请输入产品类型名称",
	});
	//硬件版本名称输入框
	$("#user_productType_hardwareVersion_add").textbox({
		required:true,
		missingMessage:"请输入硬件版本",
	});
	//软件版本名称输入框
	$("#user_productType_softwareVersion_add").textbox({
		required:true,
		missingMessage:"请输入软件版本",
	});
	
	//mac数量名称输入框
	$("#user_productType_macNumber_add").numberbox({
		required:true,
		...
		buttons:[{
			...
			handler:function(){
				if($('#user_productType_add').form('validate')){
					var productTypeName_add=$('#user_productType_productTypeName_add').val();
					var hardwareVersion_add=$('#user_productType_hardwareVersion_add').val();
					var softwareVersion_add=$('#user_productType_softwareVersion_add').val();
					var macNumber_add=$('#user_productType_macNumber_add').val();
					var requestdata={
		...
		buttons:[{
			...
			handler:function(){
				if($('#user_productType_update').form('validate')){
					var id_update=$('#user_productType_id_update').val();
					var productTypeName_update=$('#user_productType_productTypeName_update').val();
					var hardwareVersion_update=$('#user_productType_hardwareVersion_update').val();
					var softwareVersion_update=$('#user_productType_softwareVersion_update').val();
					var macNumber_update=$('#user_productType_macNumber_update').val();
					var productTypeState_update=$('#user_productType_productTypeState_update').val();
					var requestdata = {
